...
def build_argparser():
    ...
    gallery.add_argument('-cf', '--change_face', default="",nargs='*',
                          help="(optional) Name of the actor you want to be replaced with another one (old).")
    gallery.add_argument('-sw', '--switch_with', default="",nargs='*',
                          help="(optional) Name of the actor you want to replace other actor (new).")
    gallery.add_argument('-pp', '--poses_path', default="", metavar="PATH",
                          help="(optional) Path to the different poses of the new actor (new).")
...
class Visualizer:
    ...
    def __init__(self, args):
        ...
        #My change 3 lines
        self.change_face_flag=False
        if args.change_face:
            assert len(args.change_face)==len(args.switch_with), \
            "The length of cf list and sw list must be equal"
            self.list_of_olds=args.change_face
            self.list_of_news=args.switch_with
            self.poses_path=args.poses_path
            self.image_path=args.fg
            self.change_face_flag=True
        # print('poses=',self.poses_path)
    ...
    #My method
    def build_pathes(self):
        # self.path_to_old=osp.join(args.fg,args.change_face+'-0.jpg')
        # self.path_to_new=osp.join(args.fg,args.switch_with+'-0.jpg')
        dic={}
        for n in self.list_of_news:
            dic[n]=osp.join(self.image_path,n+'-0.jpg')
        return dic
            
    #my method
    def change_face(self,frame,roi,keypoints,n):
        Thresh=0.13
        left_eye,right_eye,nose_tip,_,_=keypoints
        new=self.list_of_news[n]
        path_to_new=osp.join(self.image_path,new+'-0.jpg')        
        left=np.linalg.norm(left_eye-nose_tip)
        right=np.linalg.norm(right_eye-nose_tip)
        pos_files=os.listdir(self.poses_path)
        extension=[f for f in pos_files if f.startswith(new)][0][-3:]
        # print('tmp=',tmp)
        if abs(left-right)<Thresh:
            img2read=path_to_new
        elif right>left:
            img2read=osp.join(self.poses_path,new+'_left.'+extension)
        elif left>right:
            img2read=osp.join(self.poses_path,new+'_right.'+extension)
        # print('img=',img2read)
        try:
            pic=cv2.imread(img2read)
            img=cv2.resize(pic, (int(roi.size[0]),int(roi.size[1])))
            frame[int(roi.position[1]):int(roi.position[1])+int(roi.size[1]),int(roi.position[0]):int(roi.position[0])+int(roi.size[0])]=img
        except ValueError:
            pass
        except IndexError:
            pass
    ...
    def draw_detections(self, frame, detections):
        for roi, landmarks, identity in zip(*detections):
            label=self.draw_detection_roi(frame, roi, identity)
            keypoints=self.draw_detection_keypoints(frame, roi, landmarks)
            #My 2 line
            if self.change_face_flag:
                if label in (self.list_of_olds):
                    self.change_face(frame,roi,keypoints,self.list_of_olds.index(label))
    ...
    def open_output_stream(path, fps, frame_size):
        ...
        if path != "":
            ...
            # output_stream = cv2.VideoWriter(path,0x00000021, fps, (int(frame_size[0]/2),int(frame_size[1]/2)))
